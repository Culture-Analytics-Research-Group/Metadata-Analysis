/*
	Modified from larsenmtl's code (https://bl.ocks.org/larsenmtl/e3b8b7c2ca4787f77d78f58d41c3da91) larsenmtl
	Modified by Ana Jofre
*/
...
// Render the chart
    chartObj.render = function () {
        ...
        // Draw Lines
        for (var y  in yObjs) {
            yObjs[y].path = chartObj.svg.append("path").datum(chartObj.data).attr("class", "line").attr("id", y +"-line").attr("d", yObjs[y].line).style("stroke", color(y)).attr("data-series", y).
        ...
        for (var y  in yObjs) {
            ...
            series.append('div').attr("class", "series-marker").attr("id",y).style("background-color", color(y))
